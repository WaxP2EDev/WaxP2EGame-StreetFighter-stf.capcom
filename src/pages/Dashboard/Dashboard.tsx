...
export const Dashboard = ({ NFTs, Assets, Account }: NFTProp) => {
  ...
  const [filters, setFilters] = useState(["All Schemas", "Name", "Rarity", "Powerscore", "Class Card"]);
  ...
  let filterData = [
    ...
    { value: "0", data: ["All Schemas", "series1", "promo"] },
  ...
  const handleChange = (event: any, index: any) => {
    ...
    for (const data of Assets) {
      ...
      //   continue;
      // }
      if (_filters[0] != "All Schemas" && _filters[0] != data.schema.schema_name) continue;
    ...
    <main className={styles.main}>
      ...
      <section className={styles.section}>
        ...
        <FormControlLabel
          sx={{ mb: 4 }}
          control={
            <Checkbox checked={backed_nft} onChange={handleChangeCheck} name="backed_nft" />
          }
        ...
        <Box>
          <Grid container spacing={0} >
            {showNFTs.map((NFT: any) => (
              <>
                <Grid key={NFT.data.img} item xl={2.4} md={4} sm={6} xs={12} style={{marginBottom:"50px"}}>
                  <NFTCard uri={uri_prefix + NFT.data.img} name={NFT.name} account={Account} assetID={NFT.asset_id} backed_tokens={NFT.backed_tokens}/>
                </Grid>
